...
export const EntryView: React.FC<EntryViewProps> = ({ onRoleSelected }) => {
  ...
  const handleGoogleAuth = async () => {
    ...
    try {
      ...
      setError("Google sign-in failed. Please ensure this domain is whitelisted in Firebase Console.");
    } finally {
      setIsSubmitting(false);
    }
  };
          ...
          <div className="bg-white rounded-3xl shadow-2xl border border-slate-100 p-8 md:p-10">
            ...
            <div className="flex p-1 bg-slate-50 rounded-xl mb-8">
              <button 
                ...
                className={`flex-1 py-3 rounded-lg text-sm font-semibold transition-all ${authMode === "login" ? "bg-white text-slate-900 shadow-sm" : "text-slate-400 hover:text-slate-600"}`}
              >
                Login
              </button>
              <button 
                onClick={() => setAuthMode("register")}
                className={`flex-1 py-3 rounded-lg text-sm font-semibold transition-all ${authMode === "register" ? "bg-white text-slate-900 shadow-sm" : "text-slate-400 hover:text-slate-600"}`}
              >
                Register
              </button>
            </div>
          ...
          <div className="bg-white rounded-[32px] shadow-2xl border border-slate-100 p-10 md:p-12">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              >
                ...
                <span className="absolute top-4 right-6 text-[8px] uppercase font-bold tracking-widest text-slate-400 bg-slate-100 px-2.5 py-1 rounded-full">Coming Soon</span>
              </div>
            </div>
          </div>
        </motion.div>
  ...
  return (
    <div className="min-h-screen flex flex-col lg:flex-row bg-[#F8FAFC] font-sans">
      {/* Laptop: Left Branding / Mobile: First Screen */}
      <motion.div 
        className={`w-full lg:w-[40%] bg-slate-900 p-10 md:p-20 flex flex-col justify-between text-white relative transition-all duration-500 lg:translate-x-0 ${mobileStep === 1 ? "-translate-x-full absolute opacity-0 lg:opacity-100 lg:relative" : "translate-x-0 opacity-100"}`}
      >
        ...
        <div className="relative z-10">
          ...
          <h1 className="text-4xl md:text-6xl font-display font-bold leading-tight mb-6">
            Grow your business <br className="hidden lg:block" /> with confidence.
          </h1>
          ...
          <Button 
            ...
            className="lg:hidden h-14 px-8 rounded-xl bg-white text-slate-900 font-bold hover:bg-slate-100"
          >
            Get Started
          </Button>
        </div>
      ...
      {/* Laptop: Right Auth / Mobile: Second Screen */}
      <div 
        className={`flex-1 flex items-center justify-center p-6 md:p-12 lg:p-20 transition-all duration-500 bg-[#F8FAFC] lg:translate-x-0 ${mobileStep === 0 ? "translate-x-full lg:translate-x-0 hidden lg:flex" : "translate-x-0 flex"}`}
      >
        <div className="w-full h-full lg:h-auto flex flex-col items-center justify-center relative">
          <button 
            onClick={() => setMobileStep(0)}
            className="lg:hidden absolute top-0 left-0 text-slate-500 font-medium flex items-center gap-2"
          >
           Back
          </button>
